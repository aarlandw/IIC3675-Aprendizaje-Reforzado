...
def q_learning_binary_mem(env, num_episodes, alpha, gamma, epsilon):
    ...
    for _ in range(num_episodes):
        ...
        while not done:
            ...
            next_obs, reward, done = env.step(env.action_space[action_idx])
            steps += 1
            
            best_next = np.max(Q[next_obs]) if not done else 0
            Q[obs][action_idx] += alpha * (reward + gamma * best_next - Q[obs][action_idx])
...
def sarsa_binary_mem(env, num_episodes, alpha, gamma, epsilon):
    Q = defaultdict(lambda: np.ones(len(env.action_space)))
    episode_lengths = []
    ...
    for _ in range(num_episodes):
        ...
        while not done:
            ...
            if next_action_idx is not None:
                target = Q[next_obs][next_action_idx]
                Q[obs][action_idx] += alpha * (reward + gamma * target - Q[obs][action_idx])
            else:
                Q[obs][action_idx] += alpha * (reward - Q[obs][action_idx])
...
def n_step_sarsa_binary_mem(env, num_episodes, alpha, gamma, epsilon, n=16):
    Q = defaultdict(lambda: np.ones(len(env.action_space)))
    episode_lengths = []
...
if __name__ == "__main__":
    ...
    env = BinaryMemory(env, num_of_bits=1)
    ...
    for run in tqdm(range(params['num_runs']), desc="Ejecutando con binary memory"):
        results['Q-learning'].append(q_learning_binary_mem(
            env, params['num_episodes'], params['alpha'], params['gamma'], params['epsilon']))

        results['SARSA'].append(sarsa_binary_mem(
            env, params['num_episodes'], params['alpha'], params['gamma'], params['epsilon']))

        results['16-step SARSA'].append(n_step_sarsa_binary_mem(
    ...
    with open('resultados_binary_memory_g1.csv', 'w', newline='') as csvfile:
    ...
    plt.title('Comparación con Binary Memory (1 bit)')
    plt.legend()
    plt.grid()
    plt.show()