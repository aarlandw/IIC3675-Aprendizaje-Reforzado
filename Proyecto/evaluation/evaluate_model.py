...
def plot_deployment_results(results, model_name="Model"):
    ...
    plt.axhline(y=results["avg_reward"], color='r', linestyle='--', 
    ...
    plt.subplot(1, 2, 2)
    plt.hist(rewards, bins=min(10, len(rewards)), alpha=0.7, color='green', edgecolor='black')
    plt.axvline(x=results["avg_reward"], color='r', linestyle='--', 
                label=f'Average: {results["avg_reward"]:.2f}')
    ...
    plt.tight_layout()
    plt.savefig(f'{model_name.lower().replace(" ", "_")}_deployment_results.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def visual_inspection(model_path, n_episodes=3):
    ...
    for episode in range(n_episodes):
        ...
        print(f"   🎯 Action range: [{min(min(a) for a in action_history):.2f}, {max(max(a) for a in action_history):.2f}]")
...
def main():
    ...
    parser.add_argument("model", nargs='?', default="best_model_optuna.zip", 
