...
class droneEnv(gym.Env):
    ...
    def _get_obs(self):
        ...
        distance_to_obstacle1 = (
            sqrt((self.x_pajaro - self.x) ** 2 + (self.y_pajaro - self.y) ** 2) / 500
        )
        angle_to_obstacle1 = np.arctan2(self.y_pajaro - self.y, self.x_pajaro - self.x)
        angle_obstacle1_and_velocity = np.arctan2(
            self.y_pajaro - self.y, self.x_pajaro - self.x
        ) - np.arctan2(self.yd, self.xd)
        distance_to_obstacle1 = (
            sqrt((self.x_pajaro - self.x) ** 2 + (self.y_pajaro - self.y) ** 2) / 500
        )
        ...
        distance_to_obstacle2 = (
            sqrt((self.x_pajaro2 - self.x) ** 2 + (self.y_pajaro2 - self.y) ** 2) / 500
        )
        angle_to_obstacle2 = np.arctan2(
            self.y_pajaro2 - self.y, self.x_pajaro2 - self.x
        )
        angle_obstacle2_and_velocity = np.arctan2(
            self.y_pajaro2 - self.y, self.x_pajaro2 - self.x
        ) - np.arctan2(self.yd, self.xd)
        distance_to_obstacle2 = (
            sqrt((self.x_pajaro2 - self.x) ** 2 + (self.y_pajaro2 - self.y) ** 2) / 500
    ...
    def step(self, action):
        ...
        for _ in range(5):
            ...
            if (
                self.x_pajaro < 0
                or self.HEIGHT < self.x_pajaro
                or self.y_pajaro < 0
                or self.WIDTH < self.y_pajaro
            ):
                self.a_pajaro, self.b_pajaro = randrange(-3, 3), randrange(0, 800)
                self.x_pajaro = 0
                self.y_pajaro = self.a_pajaro * self.x_pajaro + self.b_pajaro
            ...
            if (
                self.x_pajaro2 < 0
                or self.HEIGHT < self.x_pajaro2
                or self.y_pajaro2 < 0
                or self.WIDTH < self.y_pajaro2
            ):
                self.a_pajaro2, self.b_pajaro2 = choice(
                    [x * 0.1 for x in range(-10, 11)]
                ), randrange(-800, 800)
            ...
            dist_obst1 = sqrt(
                (self.x - self.x_pajaro) ** 2 + (self.y - self.y_pajaro) ** 2
            )
            dist_obst2 = sqrt(
                (self.x - self.x_pajaro2) ** 2 + (self.y - self.y_pajaro2) ** 2
            )
            ...
            self.reward += (dist_obst1 + dist_obst2) / (100 * 80)
            if dist < 50:
